...
class action_count:
    ...
    def _classify_time(self , list , userId):
        """
        将数据根据操作次数分类放入不同的文件夹
        :type userId: 传入设备ID，用于为文件起名字
        :type list: 传入用户操作列表，包含操作行为和时间
        """
        for j in class_list:  # 遍历分段的数值判断是否符合条件
            if len(list) < j:
                ...
                with open('%s/%s.txt' % (_paht , userId) , 'w' , encoding='utf-8') as _save:
                    for i in list:
                        _time =datetime.strptime(i[ 'created_at' ] , "%Y-%m-%d %H:%M:%S").time()#时间提取time，删除日期
                        _save.write(str(_time))
                        _save.write('\t\t')   #进位符，方便放入表格中
    ...
    def user_to_list(self,list):
        ...
        for i_list in list:
            ...
            self._classify_time(_one_list , i_list)
