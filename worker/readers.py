...
class get_show_list:
    ...
    def _parse(self):
        try:  # 如果被反扒，则解析json时会出错
            _response = requests.post(url=self.post_url, headers=self.headers, proxies=self.proxy_dict, data=self.data,
                                      verify=True)  # verify是否验证服务器的SSL证书)

            print(_response)
            if _response.status_code == 200:
                print('解析show_list成功：%s' % _response.status_code)
            else:
                print('解析show_list返回值异常：%s' % _response.status_code)
                return None
        except Exception as e:
            print('解析show_list失败：%s' % e)
            return None
