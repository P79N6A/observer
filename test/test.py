...
def _string():
    s = '  d  '
    s.lstrip()  # 这个字符串方法，会删除字符串s开始位置前的空格。
    s.rstrip()  # 这个内置方法可以删除字符串末尾的所有空格，看下面演示代码：
    s = "   iplaypython    "  # 有的时候我们读取文件中的内容，每行2边都有空格，能不能一次性全部去掉呢，字符符有一个内置的strip()方法可以做到。
    s.strip()
...
def _datetime():
    ...
    datetime_str = '2019-02-09 12:00:06'
    _datetime_str = datetime.strptime(datetime_str, "%Y-%m-%d %H:%M:%S")
    date_str = '2018-02-20'
    time_str = '13:53:59'

    print(datetime.strptime(date_str, "%Y-%m-%d").date())
    print(datetime.strptime(time_str, "%H:%M:%S").time())
    print(datetime.strptime(datetime_str, "%Y-%m-%d %H:%M:%S").date())

    start_time = datetime.strptime('2018-02-20', '%Y-%m-%d')
    end_time = datetime.strptime('2018-03-20', '%Y-%m-%d')
    print((start_time - end_time).days)

    # 今天
    _now = datetime.now()
    _length_time = _now - _datetime_str
    print('今天日期：%s' % _now.date())
    print('今天时间：%s' % _now.time())

    print('今天距离(%s)有：%s秒' % (_datetime_str, _length_time.seconds))
...
def find():
    ...
    test2 = [{'标题': '禾空间儿童剧嘉年华十次卡演出票', '副标题': '多部儿童剧', '地点城市': '武汉', '演出场所': '禾空间小剧场', '演出时间': '2018.10.01-2019.02.28',
             ...
             {'标题': '禾空间原创亲子互动剧《神奇动物在这里》', '副标题': '本剧由《格林童话》中《金鸟》、《牧鹅女》、《金鹅》三个故事略加改编而成', '地点城市': '武汉',
              '演出场所': '禾空间小剧场',
             ...
             {'标题': '江湖爆笑喜剧《萨瓦迪大咖》', '副标题': '一条暧昧微信引发婚姻信任危机！', '地点城市': '武汉', '演出场所': '湖北剧院',
              '演出时间': '2019.01.16-02.09',
...
def to_url():
    ...
    it = ['http://mini.eastday.com/a/180714221632914-2.html',
          'http://image.baidu.com/search/index?tn=baiduimage&ct=201326592&lm=-1&cl=2&ie=gb18030&word=%D0%D3%C8%CA%B6%B9%B8%AF%D0%C4&fr=ala&ala=1&alatpl=adress&pos=0&hs=2&xthttps=000000',
          'http://www.piaoniu.com/activity/65867', 'https://m.piaoniu.com/activity/65867',
          'http://piao.jd.com/114927.html', 'https://piao.damai.cn/161166.html',
          'http://blog.sina.com.cn/s/blog_6d9713960102yjm3.html',
          'http://www.dahepiao.com/news1/yanchu/2018120958827.html', 'http://www.nuomi.com/deal/c00vtx214.html',
          'https://www.228.com.cn/ticket-448679229.html',
          'http://epaper.oeeee.com/epaper/A/html/2019-01/12/content_1300.htm',
          'http://www.51goupiao.com/t_10839.html', 'http://m.dahepiao.com/news/2018122059666.html',
          'http://www.yidianzixun.com/article/0J7c4Uf5',
          'https://jingyan.baidu.com/article/17bd8e52ff4ed385ab2bb8c6.html', 'http://www.ypiao.com/t_62898/',
          'https://www.meishij.net/tianpindianxin/xingrendoufu_11.html', 'http://www.dianping.com/toutiao/75855435',
          'https://www.xinshipu.com/zuofadaquan/6752/', 'https://m.nuomi.com/w/expired/p00vuaieu',
          'http://blog.sina.com.cn/s/blog_4dbc922e0102ykor.html', 'http://heju.jinciwei.cn/426401.html',
          'http://www.dahepiao.com/yanchupiaowu1/hj/2018120858779.html',
          'http://wemedia.ifeng.com/88288519/wemedia.shtml', 'http://mini.eastday.com/a/181117184905524-3.html',
          'https://www.xiaohongshu.com/discovery/item/598c206ea9b2ed283b192e0d',
          'https://www.228.com.cn/ticket-508914816.html',
          'https://passport.weibo.com/visitor/visitor?entry=miniblog&a=enter&url=https%3A%2F%2Fweibo.com%2F1762768245%2FHbnIWyCi7&domain=.weibo.com&ua=php-sso_sdk_client-0.6.28&_rand=1547273333.9928',
          'http://bangumi.tv/person/14879', 'https://jingyan.baidu.com/article/ed15cb1b2b9f2f1be2698149.html']
    print(len(it))
...
def data_db():
    import pymongo
    data_table = pymongo.MongoClient('mongodb://localhost:27017/')["swarm"]['test']

    users = {'id': 1, "name": "zhangsan", "age": 18}
    dd = data_table.insert_one(users)
    print(dd.inserted_id)  # 增加多个数据
...
def sorting():
    ...
    listd.sort()  # 按第一个元素排序
    # listd.sort(key=lambda x:x[1])#升序
    # listd.sort(key=lambda x:x[1], reverse=True)#降序
    ...
    #         a += 1
    #
    # ddd= sorted(listd,key=lambda x:x[1], reverse=True)
    #
    # print(ddd)

    # d = {'启动APP': 7707, '社区首页': 36235, '消息页面': 10838, '帖子详情-回复按钮': 2405, '帖子详情-返回按钮': 35432, '首页': 14339, '社区-最新页签': 2847, '帖子详情': 57561, '选择发帖类型': 673, '发帖界面-发布按钮': 1158, '评论界面-回复': 8701, '我的界面': 6035, '反馈界面': 254, '首页-推文': 3710, '返回APP': 13923, '请求数据时': 150891, '返回桌面': 11985, '社区-文章': 13130, '用户名片界面': 13029, '社区-发帖按钮': 256, '帖子编辑页面': 914, '发帖界面-添加页签': 131, '选择标签': 301, '帖子详情-评论区回复': 5820, '帖子详情-评论按钮': 6257, '社区-消息按钮': 1, '搜索页面': 1678, '社区-搜索按钮': 324, '搜索结果': 5931, '社区-热门版块': 779, '版块页面-全部按钮': 3620, '试管婴儿版块': 4471, '版块页面-帖子': 4350, '帖子详情-评论区点赞': 21, '首页-悬浮按钮': 198, '喜报版块': 738, '社区-精华页签': 164, '个人中心-帖子': 1623, '个人中心-我的关注': 53, '关注界面': 91, '社区-全部页签': 392, '版块页面-最新按钮': 5311, '版块页面-精华按钮': 1619, '个人中心-我的粉丝': 22, '粉丝界面': 32, '举报界面': 13, '版块页面-热门按钮': 1443, '弹出框提示': 1261, '社区-热门页签': 49, '用户设置': 142, '版块页面-发贴按钮': 139, '帖子详情-分享按钮': 240, '分享帖子': 205, '首页-专题': 1024, '帖子详情-点赞按钮': 2245, '帖子详情-收藏按钮': 169, '设置界面': 367, '手机登录/注册': 513, '社区-全部版块': 71, '版块页面-置顶贴': 17, '首页-轮播图': 426, '帖子详情-功能按钮': 76, '童梦母婴版块': 776, '社区-广告': 34, 'themeView': 46, '评论界面-删除': 17, '妈妈日记版块': 165, '关于我们': 13, '帖子详情-关注/取关按钮': 33, '用户协议': 7, '账号登陆': 57, '忘记密码': 6, 'themeShare': 4, '不孕不育版块': 307, 'sitePostEdit': 53, '帖子详情-删除': 26, '修改头像': 18, 'changePhoto': 1, '全部版块': 96, '全部版块-版块按钮': 57, '医疗信息版块': 47, '全部版块-返回按钮': 21, '修改昵称': 16, '修改密码': 3, '更改手机': 2, '社区活动版块': 15, '社区服务版块': 5, 'reportTimes': 7, '评论界面-举报': 9, '更换邮箱': 2, '评论界面-点赞': 2, '人工授精版块': 22, 'forgetPassword': 1}
    # sorted_key_list = sorted(d,key=lambda x:d[x])  # 正向排序
    # # sorted_key_list = sorted(d, key=lambda x:d[x], reverse=True)      #逆向排序
    # print(sorted_key_list.i)
    #
...
# 数组类型转换
def dgbbc():
    i = {'消息页面': 665, '社区首页': 2356, '启动APP': 488, '帖子详情-回复按钮': 209, '帖子详情-返回按钮': 2376, '首页': 853, '社区-最新页签': 214,
         '帖子详情': 3463, '选择发帖类型': 40, '发帖界面-发布按钮': 94, '评论界面-回复': 514, '我的界面': 398, '帖子详情-分享按钮': 16, '分享帖子': 13,
         '社区-精华页签': 12, '用户设置': 4, '社区-全部页签': 36, '社区-热门页签': 3, 'sitePostEdit': 1, '选择标签': 12, '用户名片界面': 847,
         '请求数据时': 9645, '首页-推文': 159, '返回桌面': 774, '返回APP': 906, '社区-热门版块': 60, '版块页面-全部按钮': 207, '试管婴儿版块': 261,
         '版块页面-帖子': 229, '社区-文章': 811, '搜索页面': 128, '帖子编辑页面': 65, '个人中心-帖子': 111, '帖子详情-评论区点赞': 2, '版块页面-最新按钮': 320,
         '版块页面-精华按钮': 100, '帖子详情-评论区回复': 440, '社区-全部版块': 2, '全部版块': 5, '全部版块-版块按钮': 4, '妈妈日记版块': 13, '童梦母婴版块': 69,
         '版块页面-置顶贴': 3, '首页-专题': 71, '首页-悬浮按钮': 14, '喜报版块': 72, '社区-发帖按钮': 8, '帖子详情-评论按钮': 465, '帖子详情-收藏按钮': 9,
         '个人中心-我的粉丝': 3, '粉丝界面': 5, '个人中心-我的关注': 2, '关注界面': 4, '帖子详情-功能按钮': 6, '版块页面-热门按钮': 95, '弹出框提示': 57,
         '搜索结果': 292, '帖子详情-点赞按钮': 165, '社区-搜索按钮': 23, '版块页面-发贴按钮': 14, '设置界面': 29, '帖子详情-删除': 1, '社区-广告': 1,
         '反馈界面': 16, '社区服务版块': 1, '不孕不育版块': 1, '手机登录/注册': 25, '帖子详情-关注/取关按钮': 1, '账号登陆': 3, '用户协议': 1, '发帖界面-添加页签': 2,
         '首页-轮播图': 32}

    # print(tuple(set(i.items())))
    # print(tuple(i.items()))
    print(list(i.items()))
    # print(list(i))
    print(type(list(i.items())[0]))
...
def aazzd():
    ...
    import jieba
    # #载入自定义词库
    jieba.load_userdict("../tm51/res/user_dict.txt")

    # 全模式
    text = "我来到北京清华大学11月20000汉"
...
def retest():
    ...
    # with open(r'res/test.txt','r') as file:
    #     txt = file.read()
    # p = re.compile(r'<row>((?:.|\n)*?)</row>')  # 提取row的列表
...
def read_file():
    import os
    with open('res/d.txt', 'w') as file:
        file.write(u'1gfg\r2fdsg\r3sfgds\r4gsdgsdf\r5gsdfgsdfg\r6gdfsg\r7sfd\r8gsdfgsdfg\rfgdfs\r')

    with open('res/d.txt', 'r') as file:
        _file = file.read()
...
def ddd():
    ...
    #
    # b: bytes
    # python3.x里默认的str是(py2.x里的)
    # unicode,bytes是(py2.x)
    # 的str,b”“前缀代表的就是bytes
    # python2.x里,b前缀没什么具体意义， 只是为了兼容python3.x的这种写法


def dds():
    x_list = ['首页', '社区首页', '消息页面', '个人中心-帖子', '用户名片界面', '试管婴儿版块', '喜报版块', '搜索结果', '童梦母婴版块', '社区-全部版块', '社区-精华页签',
              '社区-最新页签',
              '社区-热门版块']
...
def dees():
    ...
    # bot = Bot()
    bot = Bot(console_qr=True, cache_path=True)  # 保留缓存自动登录。可用
    """
    cache_path –
    ...
    my_friend = bot.friends().search('小元')[0]
    my_love = bot.friends().search('晴牵一线')[0]
    # # 发送文本
    ...
    @bot.register(chats=[Friend], msg_types=None, except_self=True, run_async=True, enabled=True)
    ...
    @bot.register(chats=[my_group], msg_types=None, except_self=True, run_async=True, enabled=True)
    def forward_message(msg):
        print('[接收]' + str(msg))
        print('[接收]' + msg.txt)
        # if len(str(msg)) > 1:
    ...
    embed()  # 堵塞线程，并进入 Python 命令行，使得我们可以在调试模式中互动
